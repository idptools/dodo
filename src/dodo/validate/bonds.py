...
Provenance = Literal["rebuilt", "input", "unknown"]
...
def _residue_provenance(structure: Structure) -> np.ndarray:
    """Per-residue provenance codes: 0 unknown, 1 input (folded), 2 rebuilt (IDR or loop)."""
    codes = np.zeros(structure.n_residues, dtype=np.int8)
    for domain in structure.domains:
        span = domain.span
        codes[span.start : span.stop] = 2 if domain.kind.value == "idr" else 1
        for loop in domain.loops:
            codes[loop.start : loop.stop] = 2
    return codes
